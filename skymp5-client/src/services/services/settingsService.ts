...
export class SettingsService extends ClientListener {
  ...
  public async getServerMods(): Promise<Mod[]> {
    const serverHttpUrl = this.sp.settings["skymp5-client"]["server-http-url"];
    if (serverHttpUrl !== undefined && (
      typeof serverHttpUrl !== "string" || !/^https?:\/\/[^/\s?#]+(?:\/[^\s?#]*)?$/.test(serverHttpUrl)
    )) {
      throw new Error("server-http-url must be an absolute HTTP(S) base URL");
    }
    const manifestClient = typeof serverHttpUrl === "string"
      ? new HttpClient(this.normalizeUrl(serverHttpUrl)) as IHttpClientWithCallback
      : this.makeMasterApiClient();
    const masterKey = this.getServerMasterKey();
    const manifestPath = typeof serverHttpUrl === "string"
      ? "/manifest.json"
      : `/api/servers/${masterKey}/manifest.json`;
    let lastError: unknown;

    for (let attempt = 0; attempt < 5; ++attempt) {
      try {
        printConsole(`Trying to get server mods, attempt ${attempt}`);
        const res = await manifestClient.get(manifestPath);
        if (res.status != 200) {
        ...
        if (!Array.isArray(manifest.mods)) {
          throw new Error("server manifest does not contain a mod list");
        }
    ...
    throw new Error(`Could not verify server mod list: ${lastError}`);
  };
